...
// console.log(mainLocation);
// console.log(makeWeatherBlock("berlin", "de", "metric", "col-3"));

const mainLocation = new User("munich", "de", "metric");
    ...
    try {
      if (i === 0) {
        ...
        y.map(async (x, index) => {
          ...
          // const getMain = await makeWeatherBlock(
          //   "munich",
          //   "de",
          //   "metric",
          //   "col-3"
          ...
          if (arrHtml.length === y.length) return arrHtml;

          // elementContainer.appendChild(rowWeather);
          // console.log(arrHtml.sort((a, b) => a.position + b.position));
        });
        // arrFinal.concat(arrHtml);
        // console.log(arrFinal);
        // console.log(arrHtml.sort((a, b) => a.position + b.position));
      } else if (i > 0) {
        // // create row
        // const rowWeather = document.createElement("div");
        // rowWeather.className = `row`;
        // y.map(async (x, index) => {
        ...
        //     "row-one"
        //   );
        //   elementContainer.appendChild(get);
        // });
        // elementContainer.appendChild(rowWeather);
      }
    } catch (error) {
